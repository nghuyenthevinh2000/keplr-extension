...
export class PrivKeyEthSecp256k1 {
  ...
  toBytes(): Uint8Array {
    return new Uint8Array(util.toBuffer(this.wallet.privateKey));
  }

  getPubKey(): PubKeyEthSecp256k1 {
    //@ts-ignore
    return new PubKeyEthSecp256k1(this.wallet);
  }

  sign(msg: Uint8Array): Uint8Array {
    const secp256k1 = new EC.ec("secp256k1");
    const key = secp256k1.keyFromPrivate(this.wallet.privateKey);
...
export class PubKeyEthSecp256k1 {
  ...
  toBytes(): Uint8Array {
    return new Uint8Array(util.toBuffer(this.getAddressString()));
  }
